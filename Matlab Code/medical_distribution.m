function [DR , AS] = medical_distribution(PN , k, nurse_n, doctor_n, expert_n, check_n)
...
% 病人恶化率
sigma_0 = 0.01*betarnd(2,2,PN,1);% 服从分布~0.01*BETA(2,2)
sigma = k * sigma_0;
...
AI = exprnd(1/lamda, PN-1, 1);% 相邻病人的到达时间间隔服从1/lamda的指数分布(N个人有N-1个间隔)
AI = roundn(AI,-1);
AI(AI <= 0) = 0.1;% 相邻病人的最小时间间隔为0.1tick

% 初始化病人到达时刻
patient.arrival = zeros(PN,1);
for i = 1 : PN-1
    patient.arrival(i+1) = sum(AI(1:i));
end

% 初始化病人状态
patient.status = zeros(PN,1);% 病人所处状态[0:未进入就诊;1:正在就诊;2:结束就诊]
patient.H0 = random('uniform',0.5,1,PN,1);% 病人初始生理状态
patient.exit = zeros(PN,1);% 病人完成就诊时刻
patient.ifcheck = zeros(PN,1);% 病人是否进行过检查，0为否，1为是
patient.ifdead = zeros(PN,1);
patient.dOe = zeros(PN,1);% 病人之前被医生还是专家检查，1为医生，2为专家；以便检查后重新排队

% 初始化护士服务时间
miu_n = 2.4;% 护士服务时间均值
sigma_n = sqrt(1.29);% 护士服务时间标准差
ST_n = normrnd(miu_n, sigma_n, PN,1);
ST_n = roundn(ST_n,-1);
ST_n(ST_n <= 0) = 0.1;
...
ST_d = normrnd(miu_d, sigma_d, PN,1);
ST_d = roundn(ST_d,-1);
ST_d(ST_d <= 0) = 0.1;
...
ST_e = normrnd(miu_e, sigma_e, PN,1);
ST_e = roundn(ST_e,-1);
ST_e(ST_e <= 0) = 0.1;
...
ST_c = normrnd(miu_c, sigma_c, PN,1);
ST_c = roundn(ST_c,-1);
ST_c(ST_c <= 0) = 0.1;
...
while sum(patient.status) < 2*PN% 循环至所有病人均结束就诊流程
    % 检查病人死亡情况，如有死亡则该病人结束就诊
    patient_real = find(patient.status == 1);
    RH = calculate_RH(patient_real,patient,sigma,T);% 计算T时刻下病人健康状况
